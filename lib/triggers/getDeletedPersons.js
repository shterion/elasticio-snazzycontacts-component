...
async function getDeletedPersons(options, snapshot = {}) {
  ...
  try {
    ...
    if (!persons.content || !Array.isArray(persons.content)) {
     throw `Expected records array. Instead received: ${JSON.stringify(persons.content)}`;
    }
...
(async function() {
  try {
    ...
    getDeletedPersons(requestOptions, snapshot)
      .then((res) => {
